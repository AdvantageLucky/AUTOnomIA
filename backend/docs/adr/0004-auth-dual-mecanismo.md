...
dañar o cambiar, se necesita poder **revocar el acceso de inmediato** desde el dashboard sin esperar
a que expire un token — algo que un JWT puro no permite sin infraestructura adicional (blacklist,
TTL corto + refresh, etc.).

También se evaluó modelar el kiosko como una entidad `Kiosko` separada de `Acceso`, pero en la
Propuesta Técnica una "instancia de kiosko" mapea 1 a 1 con lo que ya es `Acceso` (ej. "Entrada A",
...
- **Kiosko** (`POST /auth/acceso/login`): la credencial (`ClaveKiosko`, hash bcrypt) vive
  directamente en el modelo `Acceso`, no en una entidad separada. El login abre una
  `SesionAcceso` persistida en la tabla `sesion_accesos`, con un token opaco de alta entropía
  (no JWT). El middleware `auth.RequireAcceso` valida ese token contra la tabla y mete el
  `acceso_id` de la sesión en el contexto. `POST /auth/acceso/{id}/revocar` (protegido con
  `RequireAdmin`, validando que el Acceso pertenece al admin autenticado) revoca todas las
  sesiones activas de ese Acceso de inmediato.

Ambas llaves de contexto (`admin_id`, `acceso_id`) viven en `internal/platform/ctxkeys/`, un
paquete sin dependencias, para que `acceso/`, `admin/` y `visitantes/` puedan leerlas en sus
handlers sin crear un ciclo de imports con `auth/` (que a su vez depende de `acceso/` y `admin/`
para resolver el login).

## Consequences
- Las rutas de dashboard (`/admins`, `/accesos`, `/visitantes` planas) quedan protegidas con
  `RequireAdmin`; las rutas anidadas de registro de visitantes (`/accesos/:id/visitantes`) quedan
  protegidas con `RequireAcceso`, verificando además que el `:id` de la URL coincide con el
  `acceso_id` de la sesión — un kiosko logeado en el Acceso 3 no puede operar sobre el Acceso 5
  solo cambiando la URL.
- Revocar un kiosko robado/perdido es inmediato (un `UPDATE` en `sesion_accesos`), a costa de una
